...
public sealed partial class WeatherAggregationService : IWeatherAggregationService
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="WeatherAggregationService"/> class.
    /// </summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (Exception exception) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            return new WeatherProviderSnapshot
            {
                ...
                SupportsMarineData = provider.SupportsMarineData,
                IsSuccess = false,
                RetrievedAtUtc = _timeProvider.GetUtcNow(),
                RawPayloadJson = string.Empty,
                QualityScore = 0D,
        ...
        DateTimeOffset refreshAttemptUtc)
    {
        ...
        var observationTimeUtc = successfulSnapshots
            .Where(static snapshot => snapshot.ObservationTimeUtc is not null)
            .Select(snapshot => snapshot.ObservationTimeUtc)
            .Max();
        ...
        return new WeatherSnapshot
        {
            ...
            Island = site.Island,
            AirTemperatureC = airSource?.AirTemperatureC,
            WaterTemperatureC = waterSource?.WaterTemperatureC,
            WindSpeedMps = windSpeedSource?.WindSpeedMps,
            WindDirectionDeg = windDirectionSource?.WindDirectionDeg,
            WaveHeightM = waveSource?.WaveHeightM,
            SeaStateText = seaTextSource?.SeaStateText,
            ...
            IsStale = false,
            ProviderSnapshots = allProviderSnapshots,
        };
    }
        ...
        DateTimeOffset refreshAttemptUtc)
    {
        return new WeatherSnapshot
        {
            ...
            DiveSiteName = site.Name,
            Island = site.Island,
            ConditionStatus = SeaConditionStatus.Unknown,
            ConditionSummary = "No provider data available",
            LastUpdatedUtc = refreshAttemptUtc,
            LastRefreshAttemptUtc = refreshAttemptUtc,
